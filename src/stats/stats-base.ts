...
//#region Public API
    ...
    public getStatsAsync(level: string | number, payload?: GetStatsPayload): Promise<Stats[]> {
        apiCall(WORTAL_API.STATS_GET_STATS);
    ...
    public postStatsAsync(level: string | number, value: string | number, payload?: PostStatsPayload): Promise<void> {
        apiCall(WORTAL_API.STATS_POST_STATS);
...
//#region Implementation interface
    ...
    protected abstract postStatsAsyncImpl(level: string | number, value: string | number, payload?: PostStatsPayload): Promise<any>;
...
//#region Validation
    ...
    protected validateGetStats(level: string | number, payload?: GetStatsPayload): ValidationResult {
        if (!isValidString(level) && !isValidNumber(level)) {
            return {
                valid: false,
                error: invalidParams(undefined, WORTAL_API.STATS_GET_STATS, API_URL.STATS_GET_STATS),
            };
        }
    ...
    protected validatePostStats(level: string | number, value: string | number, payload?: PostStatsPayload): ValidationResult {
        if ((!isValidString(level) && !isValidNumber(level)) ||
            (!isValidString(value) && !isValidNumber(value))) {
            return {
                valid: false,
                error: invalidParams(undefined, WORTAL_API.STATS_POST_STATS, API_URL.STATS_POST_STATS),
            };
        }
